...
export class ChartComponent implements OnInit, AfterViewInit {
  ...
  getFilterDate()  {
    ...
    if(this.dateDebut) {
      filter.push(
        new Filter('date_reception', this.dateDebut, 'gte')
      )
    }
    if(this.dateFin) {
      filter.push(
        new Filter('date_reception', this.dateFin, 'lte')
      )
    }
    return filter;
  }
      ...
      comp.chartSelect.subscribe(
        (data: ChartFormData) => {
          ...
          this.chartConfig.chartFormData.datasets.forEach(dataset => {
            ...
            let libelle = 'Sortie en bon de carburant';
  ...
  setLabels(qualite: QualiteEnum): any
  {
    switch(qualite) {
      ...
      case QualiteEnum.mois:
        ...
        break;
        case QualiteEnum.automobile:
          this.service.allAutomobiles$.subscribe(
                (data)=> {
        ...
        break;
        case QualiteEnum.engagement:
          this.service.allTypeEngagements$.subscribe(
                (data)=> {
                  this.chartConfig.labels = data.map(element => element.libelle);
                }
              );
