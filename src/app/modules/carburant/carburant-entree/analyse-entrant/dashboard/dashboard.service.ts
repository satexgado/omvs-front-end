...
export class DashboardService {
  ...
  private _cacheApprovisionnements$: Observable<any>;
  ...
  list(queryOptions: QueryParameter[]): Observable<Result[]> {
    return this.httpClient
      .post(`${this.url}/transport/bon-carburant-entrees/analyses-datas`,queryOptions)
...
get allCoordonnees$()  {
  ...
  get allApprovisionnements$()  {
    const queryOptions = new QueryOptions();
    queryOptions.sort = [new Sort('libelle','asc')];
    if (!this._cacheApprovisionnements$) {
      this._loading$.next(true);
        this._cacheApprovisionnements$ = new BonApprovisionnementFactory().list().pipe(
          shareReplay(1),
          map(data => data.data)
        );
    }
    return this._cacheApprovisionnements$;
  }
    ...
    const queryOptions = new QueryOptions([
      {
        or: false, filters: [
          ...
          new Filter('module', 'bon carburant entrant', 'eq'),
        ]
      },
    ]);
