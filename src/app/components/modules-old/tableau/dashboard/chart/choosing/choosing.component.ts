...
import { ChartType, ValeurEnum, ChartFormData, ChartFormDataset, ChartFormFilter, QualiteEnum } from '../enum-interface';
...
export class ChoosingComponent implements OnInit {
  ...
  chartSelectItems = [
    ...
    {id: ChartType.scatter, libelle: '', icon_class: "fas fa-braille fa-lg m-2"},
  ];
  ...
  setFormData(formData: ChartFormData) {
    ...
    const datasetFormArray = this.formbuilder.array(formData.datasets.map(dataset => {
        ...
        const datasetGrp = this.formbuilder.group({
          libelle: new FormControl(dataset.libelle,Validators.required),
